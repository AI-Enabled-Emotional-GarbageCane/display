...
usage() {
  ...
  printf '  --playback-device DEVICE  Play confirmations through this AGX audio device. Default: HDMI monitor sink\n'
}
...
play_wav() {
  ...
  if [[ -n "$PLAYBACK_DEVICE" ]]; then
    if [[ "$PLAYBACK_DEVICE" == alsa_output.* || "$PLAYBACK_DEVICE" == bluez_output.* || "$PLAYBACK_DEVICE" == auto_null* ]]; then
      if command -v paplay >/dev/null 2>&1; then
        paplay --device "$PLAYBACK_DEVICE" "$wav_path"
      elif command -v aplay >/dev/null 2>&1; then
        aplay -q -D "$PLAYBACK_DEVICE" "$wav_path"
      else
        printf '找不到支援指定裝置的播放工具，請安裝 paplay 或 aplay。\n' >&2
        return 1
      fi
    elif command -v aplay >/dev/null 2>&1; then
      aplay -q -D "$PLAYBACK_DEVICE" "$wav_path"
      ...
      printf '找不到支援指定裝置的播放工具，請安裝 aplay 或 paplay。\n' >&2
      return 1
    fi
    return
  fi
